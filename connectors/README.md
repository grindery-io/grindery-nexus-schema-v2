...
`signature` | oneOf(`string`, array<`string`>) | yes | Signature of the event including parameter names (which are mapped to input fields by key) e.g `Transfer(address indexed from, address indexed to, uint256 value)` for ERC20 Transfer event. Multiple signatures may be specified, but indexed parameters must be exactly the same in all signatures.
